...
- **Task Sources** — a hub (groundcrew needs at least one enabled source to run):
  ...
  - **Discovered sources** (requires a groundcrew with the task-source catalog API — newer than 4.46, pending release) — every source in groundcrew's task-source catalog (e.g. **jira**, plus anything installed under `~/.config/groundcrew/task-sources/`) gets its own row automatically. Enabling writes `{ "kind": "<name>" }` and that's the whole install: groundcrew materializes the source's scripts itself on the next `crew` run. The screen live-checks the manifest's prerequisites (binaries on `PATH`) and credentials (token file or env var), shows the manifest's own install/setup instructions for whatever is missing, and lets you override the manifest's `env` defaults. On older groundcrew versions these rows simply don't appear.
